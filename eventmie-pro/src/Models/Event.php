...
use Classiebit\Eventmie\Models\PosModel;
...
class Event extends Model
{
    ...
    public function get_my_events($params = [])
    {
        return Event::select('events.*')
            ...
            ->selectRaw("(SELECT CT.name FROM categories CT WHERE CT.id = events.category_id) category_name")
            ->selectRaw("(SELECT COUNT(BK.id) FROM bookings BK WHERE BK.event_id = events.id  ) count_bookings")
            ->selectRaw("(SELECT COUNT(CK.id) FROM bookings BK INNER JOIN checkins CK ON BK.id = CK.booking_id WHERE BK.event_id = events.id AND BK.is_paid = 1 AND BK.status = 1) count_checkins")
    ...
    public function get_all_myevents($params = [])
    {
        ...
        if(Auth::check()) {
            if(Auth::user()->hasRole('pos')) {
                $params['organiser_id'] = Auth::user()->organizer_id;

                $pos_model = new PosModel();
                $event_ids = $pos_model->get_pos_event_ids($params['organiser_id'])->toArray();
            }
        }
        ...
        if(!empty($event_ids) && Auth::user()->hasRole('pos')) {
