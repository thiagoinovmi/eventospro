    ...
    */
    ...
    protected static function booted()
    {
        ...
        if(!empty(\Request::route())) {
            $exclude_bulk_scope_routes = [
                'voyager.bookings.bulk_bookings',
                'eventmie.get_booking',
                'o_complimentary_bookings',
                'organiser_complimentary_bookings',
                'event_bulk_total',

                'eventmie.scan_ticket_camera',
                'eventmie.scan_ticket_laser',
                'eventmie.verify_and_checkin',
                'eventmie.get_booking_details',
            ];
            if(!in_array(\Request::route()->getName(), $exclude_bulk_scope_routes))
            {
                static::addGlobalScope(new BulkScope);
            }
            
            $include_bulk_scope_routes = [
                'voyager.bookings.bulk_bookings',
                'o_complimentary_bookings',
                'organiser_complimentary_bookings',
                'event_bulk_total',
            ];
            if(in_array(\Request::route()->getName(), $include_bulk_scope_routes))
            {
    ...
    public function get_organiser_bookings($params = [])
    {
        ...
        $query->select('bookings.*', 'CM.customer_paid', 'E.slug as event_slug', 'E.thumbnail as event_thumbnail')
            ...
            if(Auth::user()->hasRole('pos')) {
                $query->where('pos_id', $params['pos_id']);
            }
    ...
    {
        $booking = Booking::select($select)
            ->selectRaw('(SELECT COUNT(*) FROM checkins WHERE checkins.booking_id = bookings.id) as actual_checked_in')
            ->where($params)
            ->get();
    ...
    /**
     * Get the checkins for the bookings.
     */
