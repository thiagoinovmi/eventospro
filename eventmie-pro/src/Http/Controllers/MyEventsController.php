...
class MyEventsController extends Controller
{
    ...
    public function form($slug = null, $view = 'eventmie::events.form', $extra = [])
    {
        ...
        $is_openai_api_key = setting('ai.openai_api_key') ?: env('OPENAI_API_KEY', false);
        
        return Eventmie::view($view, compact('event', 'organisers', 'organiser_id', 'extra', 'selected_organiser', 'is_openai_api_key'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'scan_on_event_day_only' => 'nullable|boolean',
        ], [
        ...
        $params = [
            ...
            "scan_on_event_day_only" => $request->scan_on_event_day_only ? 1 : 0,
        ];
        ...
        // in case of update
        if(!empty($request->event_id))
            $mail['mail_subject']   = __('eventmie-pro::em.event_updated');
    ...
    public function export_attendees($slug)
    {
        ...
        foreach($bookings as $key => $item)
        {
            ...
            $bookings[$key]['order_number']         = "'".$item['order_number']."'";
            ...
            $bookings[$key]['checked_in']           = $item['actual_checked_in'].' / '.$item['quantity'];
        }    
    ...
    protected function prepare_repetitive_event(Request $request)
    {
        ...
        $start_month = new DateTime($start_date->format('Y-m-01'));
        $end_month = new DateTime($end_date->format('Y-m-t')); // 't' gives last day of month

        // Calculate months (inclusive)
        $interval = $start_month->diff($end_month);
        $count_months = ($interval->y * 12) + $interval->m + 1; // +1 to include both months

        // Handle same month case
        if ($start_month->format('Y-m') === $end_month->format('Y-m')) {
            $count_months = 1;
        }
...
/**
    ...
    public function storeLocationEditorMedia(Request $request)
    {
        try {
            // Validate the request
            $request->validate([
                'image' => 'required|file|mimes:jpg,jpeg,png,webp|max:2048', // Limit file size to 2MB
                'field' => 'required|string|in:location', // Validate the field parameter
            ]);

            $params = [
                'file' => $request->file('image'),
                'path' => "editor_images/online_location/{$request->input('field')}/" . now()->format('F_Y'), // Dynamic path
            ];
