...
class CategoriesController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($storageDisk === 's3') {
            ...
            $iconName = time() . rand(1, 999) . '.webp';

            if ($request->hasFile('thumb')) {
                //  Process and resize the image
                $image = Image::make($request->file('thumb'))
                    ->encode('webp', 90)
                ...
                // $imageUrl = Storage::disk('s3')->url($path . $imageName);
                $imageUrl = $path.$imageName;

                // ✅ Update request data dynamically
                $request->merge(['thumb' => $imageUrl]);
            }

            if ($request->hasFile('icon')) {
                //  Process and resize the icon image
                ...
                //  Store in S3 with public visibility
                Storage::disk('s3')->put($path.$iconName, $icon);
                $iconUrl = $path.$iconName;
                $request->merge(['icon' => $iconUrl]);
            }
        }
        ...
        $category->thumb = $imageUrl != null ? $imageUrl : $category->thumb;
        $category->icon = $iconUrl != null ? $iconUrl : $category->icon;
        $category->save();
